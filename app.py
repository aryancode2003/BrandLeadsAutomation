...
# ==========================================
# BENTO GRID UI & HIGH-VISIBILITY SIDEBAR
# ==========================================
st.markdown("""
    <style>
...
# ==========================================
# 1. FILE SYSTEM & ADVANCED DATABASE SETUP
# ==========================================
...
def log_activity(email, brand, action, total_leads, fresh_leads, delivery_leads, lost_leads, file_name):
    new_log = pd.DataFrame([{
        "Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "User_Email": email, "Brand": brand, "Action": action,
        "Total_Leads": total_leads, "Fresh_Leads": fresh_leads, 
        "Delivery_Leads": delivery_leads, "Lost_Leads": lost_leads,
        "File_Name": file_name
    }])
        ...
        if st.form_submit_button("Authenticate", use_container_width=True):
            if email_input in USER_DATABASE and USER_DATABASE[email_input]["password"] == password_input:
                ...
                if user_brand == brand or user_brand == "ALL":
                    ...
                    log_activity(email_input, brand, "Logged In", 0, 0, 0, 0, "N/A")
                    st.rerun()
...
# ==========================================
    ...
    with st.sidebar:
        ...
        try:
            ...
            else: st.markdown(f"<h1 style='text-align:center; color:#3B82F6 !important; font-size:2.2rem;'>{active_brand}</h1>", unsafe_allow_html=True)
        except:
    ...
    if st.session_state.user_role == "superadmin":
        tab_proc, tab_brand_mgr, tab_logs = st.tabs(["📊 Lead Engine", "⚙️ Brand Settings", "📈 Analytics & Logs"])
    else:
        ...
        if uploaded_file is not None:
            try:
                with st.spinner("Executing Brand-Specific Logic..."):
                    try: df = pd.read_csv(uploaded_file, sep='\t', encoding='utf-16')
                    except UnicodeError:
                    ...
                    if 'phone_number' in df.columns:
                        df_transformed['Contact Phone Number'] = df['phone_number'].astype(str).str.replace(r'\D', '', regex=True).apply(lambda x: f"{x}" if pd.notnull(x) else "")
                    ...
                    if active_brand == "FATAFAT":
                        ...
                        def determine_utm_jugnoo(row):
                            ...
                            if any(k in combined_vals for k in ['driver', 'merchant', 'driving', 'job', 'delivery']): return 'as_a_delivery_boy'
                ...
                fresh_len = len(df_transformed[df_transformed['Stage'] == 'Fresh Lead'])
                lost_len = len(df_transformed[df_transformed['Stage'] == 'Lost'])
                delivery_len = len(df_transformed[df_transformed['Stage'] == 'Driver/Merchant'])

                # BENTO UI
                m_col1, m_col2, m_col3, m_col4 = st.columns(4)
                with m_col1: st.markdown(f"<div class='metric-bento'><div class='metric-icon'>👥</div><h3>Total Leads</h3><h2>{total_len}</h2></div>", unsafe_allow_html=True)
                with m_col2: st.markdown(f"<div class='metric-bento'><div class='metric-icon'>🏢</div><h3>Fresh Leads</h3><h2>{fresh_len}</h2></div>", unsafe_allow_html=True)
                with m_col3: st.markdown(f"<div class='metric-bento'><div class='metric-icon'>🛑</div><h3>Auto-Lost</h3><h2>{lost_len}</h2></div>", unsafe_allow_html=True)
                
                if active_brand == "SOLO BEAUTY":
                    with m_col4: st.markdown(f"<div class='metric-bento'><div class='metric-icon'>📉</div><h3>No Investment</h3><h2>{delivery_len}</h2></div>", unsafe_allow_html=True)
                else:
                    with m_col4: st.markdown(f"<div class='metric-bento'><div class='metric-icon'>🛵</div><h3>Delivery</h3><h2>{delivery_len}</h2></div>", unsafe_allow_html=True)
    ...
    if st.session_state.user_role == "superadmin":
        with tab_brand_mgr:
            ...
            if st.button(f"💾 Permanently Save {active_brand} Rules"):
                ...
                log_activity(st.session_state.user_email, active_brand, "Updated Brand Cities", 0, 0, 0, 0, "N/A")
    ...
    if st.session_state.user_role == "superadmin":
        with tab_logs:
            ...
            try:
                ...
                st.error(f"Error loading analytics. Note: If you just updated the script, you MUST delete the old 'usage_logs.csv' file first so the new database format can generate. Details: {e}")
